...
class Solution:
    ...
    def search_graph(self, s, e, graph, visited):
        ...
        for s2, v2 in graph[s].items():
            if s2 not in visited:
                ...
                if s != s2:
                    t = self.search_graph(s2, e, graph, visited)
                    # 찾은 경우에만 업데이트 해 준다.
                    if t != -1:
                        return t * v2
        return -1
    ...
    def calcEquation(self, equations: 'List[List[str]]', values: 'List[float]',
        ...
        # 코드에선 search_graph 의 (t * v2) 부분이다. 그리고 그 값을 'a': {'c': 6.0} 에 업데이트 한다.
        ...
        for q in queries:
            visited = set()
            t = self.search_graph(q[0], q[1], graph, visited)
            # 찾은 경우에만 시작 노드, 타겟 노드 쌍을 결과 값으로 업데이트 해 준다.
            if t != -1:
                graph[q[0]].update({q[1]: t})
